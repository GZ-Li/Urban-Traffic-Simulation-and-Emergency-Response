...
class GreedyStrategy:
    ...
    def load_traffic_flow(self):
        """加载交通流量数据"""
        print(f"    加载交通流量: {self.route_file}")
    ...
    def generate(self):
        ...
        # 未清扫道路集合（按区域）
        uncleaned_by_region = {f'region{i}': set() for i in range(1, self.num_trucks+1)}
        for edge_id in self.edge_data:
            ...
            if region in uncleaned_by_region:
                uncleaned_by_region[region].add(edge_id)
        ...
        while any(state['time_elapsed'] < max_time_seconds for state in truck_states.values()):
            for region, state in truck_states.items():
                ...
                # 查找当前区域交通流量最大的未清扫道路
                candidates = [(eid, self.traffic_flow.get(eid, 0)) 
                             for eid in uncleaned_by_region[region]]
                ...
                try:
                    ...
                    uncleaned_by_region[region].remove(target_edge_id)
                    ...
                    if consecutive_no_path[region] >= 10:
                        ...
                        if uncleaned_by_region[region]:
                            next_edge = list(uncleaned_by_region[region])[0]
                            state['current_node'] = self.edge_data[next_edge]['from']
                            consecutive_no_path[region] = 0
                            print(f"    {region} teleport to {next_edge}")
        
        # 构建输出结果
        result = {
            'strategy_name': 'greedy',
            'description': '贪心策略：优先清扫交通流量大的道路',
            'config': {
        ...
        print(f"    [贪心策略] 完成: 共清扫 {total_cleaned} 条道路")
        return result
    
    def save_result(self, output_path):
        """保存结果"""
        # 结果已在generate()中生成，这里只需调用者传入
        pass
