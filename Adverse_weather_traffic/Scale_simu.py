...
def init_sumo(route_file=None):
    sumo_cmd = [
        "sumo",
        "-c", "E:\\Traffic_Simulation\\Adverse_weather_traffic\\Core_500m_test_scale_rain.sumocfg",
    ]
...
def modify_sumocfg(input_file, out_file, scale):
    ...
    for route in root.iter('route-files'):
        route.set('value', f"scale_rou/routes_od_{str(scale)}_rain.rou.xml") 
...
def run_simulation(matched_df, duration=401, output_json_path="E:\\Traffic_Simulation\\Adverse_weather_traffic\\simulation_results\\scale_average_speeds_rain_0718.json"):
    total_res = {}
    for scale in simulation_config['scale']:
        print(scale)
        param = simulation_config['weather_params']
        modify_vehicle_xml(f"scale_rou/routes_od_{str(scale)}.rou.xml", f"scale_rou/routes_od_{str(scale)}_rain.rou.xml", param['accel'], param['decel'], param['max_speed'], param['min_gap'])
        modify_sumocfg("E:\\Traffic_Simulation\\Adverse_weather_traffic\\Core_500m_test_scale_rain.sumocfg", "E:\\Traffic_Simulation\\Adverse_weather_traffic\\Core_500m_test_scale_rain.sumocfg", scale)
        init_sumo()
        time_road_speeds = {}
